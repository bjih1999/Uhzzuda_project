...
NB_classifier.fit(training_data, training_labels[:][8])
...
f = open('shuffle/shuffled_review_sent_rhino_1126.csv', 'r')
...
count = 0
with open('classifier/temp_data2_fix.csv', 'w', newline='') as ffff:
    makewrite = csv.writer(ffff)
    for row_prediction in prediction:
        if row_prediction == 1:
            makewrite.writerow([' '.join(review_texts[count])])
        count = count + 1

count = 0
with open('classifier/temp_preprocessed_data2.csv', 'w', newline='') as ffff:
    makewrite = csv.writer(ffff)
    for row_prediction in prediction:
        if row_prediction == 1:
            makewrite.writerow(preprocessed_texts[count])
        count = count + 1
