...
ff = open('classifier/label_7000_final_ver.csv', 'r')
arr = []
label = []
...
training_data, validation_data , training_labels, validation_labels = \
    train_test_split(df_data, df_labels, test_size = 0.2, random_state = 52, shuffle=True)

for i in range(13):
    GB_classifier.fit(training_data, training_labels[:][i])

    pred = GB_classifier.predict(validation_data)
    print(i)
    print("Accuracy = ", accuracy_score(validation_labels[:][i], pred))
    print("Precision = ", precision_score(validation_labels[:][i], pred, zero_division=1, average='binary'))
    print("Recall = ", recall_score(validation_labels[:][i], pred))
    print("f1 score = ", f1_score(validation_labels[:][i], pred))
...
prediction = GB_classifier.predict(wtf)
# 너무 느리면 prediction = knn_classifier.predict(validation_data)

# 6항목을 정답으로 분류한 결과 문장 (6은 교수님+)
count = 0
with open('classifier/GBClassifier_grade+_add1.csv', 'w', newline='') as ffff:
