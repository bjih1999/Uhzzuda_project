...
MNB_classifier.fit(training_data, training_labels[:][8])

pred = MNB_classifier.predict(validation_data)
...
prediction = MNB_classifier.predict(wtf)
# 너무 느리면 prediction = knn_classifier.predict(validation_data)

# 6항목을 정답으로 분류한 결과 문장 (6은 교수님+)
count = 0
with open('classifier/MNBClassifier_class+_new.csv', 'w', newline='') as ffff:
