...
with open('konlpy/preprocessed_review_sent.csv', 'w', newline='') as output_file:
    #test 중
    ...
    for review in reviewlist:
        ...
        for morph in morphs: # morph[1].startswith('M') == False and
            if  morph[1] != 'II' and morph[1].startswith('J') == False and morph[1].startswith('S') == False and morph[1].startswith('E') == False:
                sent.append(morph[0])
            if morph[1].startswith('S') or (morph[1].startswith('E') and '고' in morph[1]):
                break;
            # if morph[1].startswith('M'):
            #     M_check.append(morph[0])
